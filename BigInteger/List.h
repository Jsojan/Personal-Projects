...
public:
    ...
    // position()
    // Returns the position of cursor in this List:
    // 0 <= position() <= length().
    int position() const;
    ...
    // the direction front-to-back) for the first occurence of element x. If x
    ...
    // cleanup()
    // Removes any repeated elements in this List, leaving only unique 
    // elements. The order of the remaining elements is obtained by retaining 
    // the frontmost occurrance of each element, and removing all other
    // occurances. The cursor is not moved with repsect to the retained 
    ...
    // by the elements of L. The cursor in the return List will be at 
    // position 0.
    ...
    // equals()
    // Returns true if and only if this List is the same integer sequence as
    // R.
